...
AutoSequelize.prototype.run = function (callback) {
  ...
  function generateText(err) {
    var quoteWrapper = '"';
    if (err) console.error(err)
    console.log(`option json===${self.options.json}`)
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      } else if (!self.options.typescript) {
      ...
      _.each(fields, function (field, i) {
        ...
        _.each(fieldAttr, function (attr, x) {
          ...
          if (attr === "foreignKey") {
            ...
            if (self.options.json) {
              text[table] += spaces + spaces + spaces + "'" + attr + "': 'DataTypes." + self.tables[table][field][attr].replace(/\'/g,'\\"');
            } else {
              text[table] += spaces + spaces + spaces + "'" + attr + "': DataTypes." + self.tables[table][field][attr];
            }
          } else {
            // console.log(`=========type=${typeof self.tables[table][field][attr]} table=${field} field=${field} attr=${attr}`)
            // _attr.toLowerCase() fixed throw boolean
            var _attr = (self.tables[table][field][attr] || '');
            if (typeof _attr === 'boolean') return
            if (typeof _attr === 'string') _attr = _attr.toLowerCase();
      ...
      // convert to json
      text[table] = text[table].replace(/true'/g, "true").replace(/false'/g, "false").replace(/'/g, '"').replace(/""/g, '"')
      text[table] = text[table].replace(/"true"/g, "true").replace(/"false"/g, "false").replace(/}"/g, '}')
      _callback(null);
    }, function () {
...
AutoSequelize.prototype.write = function (attributes, typescriptFiles, callback) {
  ...
  function createFile(table, _callback) {
    ...
    if (self.options.json) {
      fs.writeFile(path.resolve(path.join(self.options.directory, fileName + '.json')), attributes[table], _callback);
    } else {
